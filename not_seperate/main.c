...
void verticalFlip(uchar *pixels, uint32 width, uint32 height, uint32 bytesPerPixel)
{
    ...
    for (int i = 0; i < height / 2; i++) {
        for (int j = 0; j < unpaddedRowSize; j++) {
...
void horizontalFlip(uchar *pixels, uint32 width, uint32 height, uint32 bytesPerPixel)
{
    int unpaddedRowSize = width * bytesPerPixel;
    uchar tempPixel;
...
//逆時鐘轉
void rotateLeft(uchar *pixels, uint32 width, uint32 height, uint32 bytesPerPixel)
{
    int unpaddedRowSize = width * bytesPerPixel;
    uchar tempPixel;
    ...
    //我要知道要垂直轉的寬度多少
    printf("%d",unpaddedRowSize);
    //右邊兩個垂直換
    for (int i = 0; i < height / 2; i++) {
        for (int j = 150; j < unpaddedRowSize; j++) {
            tempPixel = *(pixels + i * unpaddedRowSize + j);
            *(pixels + i * unpaddedRowSize + j) = *(pixels + (height - 1 - i) * unpaddedRowSize + j);
            *(pixels + (height - 1 - i) * unpaddedRowSize + j) = tempPixel;
        }
...
//定義指標和變數以存儲像素資料、圖像寬度、高度和每像素的位元組數。
//調用 readImage 函數讀取 "img.bmp" 圖像資料。
//調用 imageProcessing 函數進行垂直鏡像處理。
//調用 writeImage 函數將處理後的圖像寫入 "img2.bmp"。
//釋放為像素資料分配的記憶體。
//返回 0，表示程式成功執行。
...
int main(void)
{
    ...
    //逆時針用下面
    //rotateLeft(pixels, width, height, bytesPerPixel);

    //writeImage("img5.bmp", pixels, width, height, bytesPerPixel);
...
//這個程式展示了如何用 C 語言讀取、處理和寫入 BMP 文件。它處理了 BMP 頭和資訊頭的讀寫，進行了像素資料的處理（垂直鏡像），並確保了記憶體管理的正確性。垂直鏡像處理函數展示了如何有效地操作像素資料。
